...
def compute_results():
    ...
    for file in instances:
        ...
        solve = solve_regret
        times, init_solutions = zip(*mp.Pool().map(solve, [(cities, i) for i in range(1)]))
        ...
        for local_search in [SearchGreedy(cities)]:
            rand_solutions = [random_solution(n) for _ in range(1)]
...
random_cycles = random_solution(200)
plt.subplots()
plot_solution(coords, random_cycles)
plt.show()
